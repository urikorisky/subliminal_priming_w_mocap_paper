effect_size_avgs <- function(p){
  ...
  for(var_name in p$VAR_NAMES){
    ...
    cat(paste('\n\n------',new_var_name,'------\n'))
    # Cohen's d
    print(cohens_d(get(new_var_name)~cond, data=temp_df, paired=FALSE, ci=NULL))
    print(cohen.d(get(new_var_name)~cond | Subject(sub), data=temp_df, paired=TRUE, ci=NULL))
