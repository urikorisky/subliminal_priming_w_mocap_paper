...
%% Parameters
% Sun 11/05/25 18:33:23 <RUN THIS>
disp("Started setting up params.")
load('../../experiment/RUN_ME/code/p.mat');
addpath(genpath('./imported_code'));

% % % URI - messing with the parameters:
p.PROC_DATA_FOLDER = '../processed_data/forSD_VSS_May11_25_nonStand_TrajNorm/';
p.DATA_FOLDER = '../../../downloaded_from_Z/raw_data/';
...
p.SUBS = p.EXP_4_1_SUBS;%p.SIM_SUBS; % to analyze.
...
p = defineParams_URI_EDIT_VSS25(p, p.SUBS(1));
...
if p.SIMULATE
    ...
    if gen_files
        ...
        for iSub = p.SUBS
            p = defineParams_URI_EDIT_VSS25(p, iSub);
...
tic
for iTraj = 1:length(traj_names)
    [reach_bad_trials, reach_n_bad_trials, reach_bad_trials_i] = trialScreen_URI_EDIT(traj_names{iTraj}, 'reach', p);
    ...
    if any(p.ORIG_SUBS < 43)
        ...
        [keyboard_bad_trials, keyboard_n_bad_trials, keyboard_bad_trials_i] = trialScreen_URI_EDIT(traj_names{iTraj}, 'keyboard', p);
    end
...
tic
for iTraj = 1:length(traj_names')
    [reach_bad_subs, reach_valid_trials] = subScreening_URI_EDIT(traj_names{iTraj}, pas_rate, 'reach', p);
    [keyboard_bad_subs, keyboard_valid_trials] = subScreening_URI_EDIT(traj_names{iTraj}, pas_rate, 'keyboard', p);
...
% % % URI - adding the MAD results as well:
% MAD.
mad_df = fVal('mad', 'reach', '', [], p);
writetable(mad_df, [p.PROC_DATA_FOLDER '/format_to_r__r_mad_' p.DAY '_' p.EXP '.csv']);
% % % /Uri
